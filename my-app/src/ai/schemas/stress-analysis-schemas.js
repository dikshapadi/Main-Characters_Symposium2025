...
export const StressAnalysisInputSchema = z.object({
  ...
  SpO2: z.number().min(88).max(100).describe("Oxygen Saturation percentage. Range: 88 to 100."),
  Steps: z.number().min(0).max(600).describe("Number of steps. Range: 0 to 600."),
  Distance: z.number().min(0.0).max(0.6).describe("Distance in km. Range: 0.0 to 0.6."),
  Calories: z.number().min(2).max(10).describe("Calories burned. Range: 2 (0 steps) to 10 (600 steps)."),
  ...
  DrinkingHabits: z.enum(["None", "Occasional", "Regular"])
    ...
    .describe("Drinking habits: None (maps to null), Occasional, Regular."),
  SmokingHabits: z.enum(["Non-smoker", "Occasional", "Regular"]).describe("Smoking habits: Non-smoker, Occasional, Regular."),
  PastMedicalHistory: z.enum(["None", "Hypertension","Other","Diabetes"]).describe("Past medical history: None, Hypertension, Other,Diabetes."),
